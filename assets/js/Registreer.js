    ...
    form.addEventListener('submit', e => {
        ...
        function SignUp() {
            FYSCloud.API.queryDatabase(
                "INSERT INTO user (isAdmin, firstName, lastName, email, password, profileImage) VALUES (?, ?, ?, ?, ?,?);",
                [0, firstname.value, lastname.value, email.value, password.value, "https://www.showflipper.com/blog/images/default.jpg"]
            ).then(() => {
                FYSCloud.API.queryDatabase(
        ...
        function checkInputs() {
            ...
            if (emailValue === '') {
               ...
               return FYSCloud.API.queryDatabase(
                    "SELECT userID FROM user WHERE email = (?)", [email.value],
                );
            }
//
